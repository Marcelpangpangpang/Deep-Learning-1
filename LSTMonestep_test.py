...
class LSTMOneStep(nn.Module):#code waarop model is gertraind
...
model = LSTMOneStep(input_size=1, hidden_size=248, num_layers=3, dropout=0.0)
model.load_state_dict(torch.load('/Users/mariskacordus/Downloads/LSTMonestep.pth'))
model.eval()
...
plt.scatter(np.arange(len(data), len(data) + len(testdata)), testdata, 
                label='Test values (200 steps)', color='green', s=1)
